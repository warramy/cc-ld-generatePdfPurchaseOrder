const drawPurchaseOrderPDF = require('./drawPurchaseOrderPDF')
...
async function genFilePDFAndUploadPDF(data) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        const fileName = `purchaseOrder_${data.poNumber}.pdf`
        const pathFile = `/tmp/${fileName}`

        drawPurchaseOrderPDF.generatePurchaseOrderFilePDF(data, pathFile).then(({ fileStatus, file }) => {
            if (fileStatus === true) {
